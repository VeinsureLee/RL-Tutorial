...
奖励拆为 4 路（info 分别暴露，主于 train/plot 分量展示）：
    step_rewards     : 每步固定时间惩罚 (-1)
    approach_rewards : 接近/远离/到达/禁区 的导航奖励（来自 env.yml）
    comm_rewards     : 通信奖励 = omega * (ber 变好/变差/平 → +ber_reward_better / -ber_reward_worse / 0)
    total (rewards)  : 上述三者之和

备注：CLI/图表输出一律英文；代码注释用中文；matplotlib 全局关闭 unicode minus 以避开 Windows 字体警告。
"""
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
class MultiRobotEnv:
    ...
    观测: 每 agent 的网格坐标 (row, col) 编码为单个整数 state_index
    ...
    def __init__(self, config: dict, randomize_on_reset: bool = False):
        ...
        self.reward_closer = config["reward_closer"]
        self.reward_farther = config["reward_farther"]
        self.reward_same = config["reward_same"]
        self.reward_forbidden = config["reward_forbidden"]
        self.reward_step = float(config.get("reward_step", -1.0))  # 每步固定时间惩罚
        ...
        # 差分通信奖励：本步 BER 相比上一步变好/变差时的定额奖励
        self.ber_reward_better = config.get("ber_reward_better", 0.5)
        self.ber_reward_worse = config.get("ber_reward_worse", -0.5)

        # 预计算无线电地图（一次性算距离/AoA/PL/β/steering + AABB LOS）
        ...
        self.prev_ber = None  # (num_agents,) 上一步 BER，NaN 表示尚无基线
            ...
            if isinstance(area, (list, tuple)):
                if len(area) == 4:
                    ...
                    if isinstance(first, (list, tuple)) and isinstance(second, (list, tuple)):
                        ...
                        size = int(second) if isinstance(second, (int, float, np.integer)) else int(second[0])
    ...
    def reset(self):
        ...
        self.prev_ber = np.full(self.num_agents, np.nan)
    ...
    def step(self, actions):
        ...
        # 记录本步是否越界（用于 boundary → forbidden 处理，与 3bede13 收敛配方一致）
        out_of_bounds = np.zeros(self.num_agents, dtype=bool)

        # 解码动作并预测下一位置（done agent 跳过）
        for i in range(self.num_agents):
            ...
            if 0 <= new_r < self.rows and 0 <= new_c < self.cols:
                ...
                # 越界：位置保持不变，但记录下，后续按 forbidden 处理
                out_of_bounds[i] = True

        # 三路奖励分量
        rewards = np.zeros(self.num_agents)
        step_rewards = np.zeros(self.num_agents)
        approach_rewards = np.zeros(self.num_agents)
        comm_rewards = np.zeros(self.num_agents)
        active_indices = np.where(~self.done_flags)[0]

        for i in active_indices:
            r, c = next_positions[i]
            step_rewards[i] = self.reward_step  # 每步固定时间惩罚

            # 越界：按 forbidden 处理（位置已经原地，无需再撤回）
            if out_of_bounds[i]:
                approach_rewards[i] = self.reward_forbidden
            ...
            if (r, c) in self.forbidden_set:
                approach_rewards[i] = self.reward_forbidden
                next_positions[i] = self.positions[i].copy()
                rewards[i] = step_rewards[i] + approach_rewards[i]
                continue

            # 到达目标
            if (r, c) == self.target_states[i]:
                approach_rewards[i] = self.reward_goal
            ...
            if new_dist < old_dist:
                approach_rewards[i] = self.reward_closer
            elif new_dist > old_dist:
                approach_rewards[i] = self.reward_farther
            else:
                approach_rewards[i] = self.reward_same
            rewards[i] = step_rewards[i] + approach_rewards[i]
        ...
        # 通信 BER 计算（对当前位置，而非 done agent）
        ber_arr = np.zeros(self.num_agents)
        sinr_arr = np.zeros(self.num_agents)
        ...
        if len(active_indices) > 0:
            ...
            for j, idx in enumerate(active_indices):
                ber_arr[idx] = ber_result["ber"][j]
                sinr_arr[idx] = ber_result["sinr"][j]

            # 差分 BER 奖励：BER 变小 → +better；变大 → +worse；首步/相等/done/禁区 → 0
            for idx in active_indices:
                if self.done_flags[idx]:
                    continue
                pos = (int(self.positions[idx, 0]), int(self.positions[idx, 1]))
                if pos in self.forbidden_set:
                    continue
                cur_ber = float(ber_arr[idx])
                prev = self.prev_ber[idx]
                if np.isnan(prev):
                    delta = 0.0
                elif cur_ber < prev:
                    delta = self.ber_reward_better
                elif cur_ber > prev:
                    delta = self.ber_reward_worse
                else:
                    delta = 0.0
                self.prev_ber[idx] = cur_ber
                comm_rewards[idx] = self.omega * delta
                rewards[idx] += comm_rewards[idx]
        ...
        return next_states, rewards.tolist(), self.done_flags.tolist(), info
    ...
    def _draw_agents(self, ax, positions, done_flags, trajectories):
        ...
        for i in range(self.num_agents):
            ...
            ax.scatter([sc], [sr], c=color, marker='o', s=100,
                       edgecolors='black', linewidths=1, zorder=5)
            tr, tc = self.target_states[i]
            ax.scatter([tc], [tr], c=color, marker='*', s=200,
                       edgecolors='black', linewidths=1, zorder=5)
            traj = trajectories[i] if trajectories is not None else self.trajectories[i]
            if len(traj) > 1:
                arr = np.array(traj)
                ax.plot(arr[:, 1], arr[:, 0], '-', color=color,
            ...
            ax.scatter([pc], [pr], c=color, marker=marker, s=80,
