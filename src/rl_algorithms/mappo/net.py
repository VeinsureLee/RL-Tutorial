"""
MAPPO 专用 Actor-Critic 网络。

每个 agent 独立的 Actor（分散执行），共享或独立的 Critic（集中训练）。
这里采用 Independent MAPPO：每个 agent 一套独立的 Actor-Critic 网络。
...
class ActorCriticNet(nn.Module):
    """Per-agent Actor-Critic 网络：共享主干，分头输出 action logits 和 state value。"""
